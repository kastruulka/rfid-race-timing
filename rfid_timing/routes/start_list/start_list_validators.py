...
def validate_category_payload(data: dict) -> tuple[dict | None, tuple | None]:
    ...
    finish_mode = _clean_optional_text(
        data.get("finish_mode") or "laps", max_length=10
    ).lower()
    if finish_mode not in {"laps", "time"}:
        return None, (jsonify({"error": "Некорректный режим гонки"}), 400)
    ...
    if finish_mode == "time":
        try:
            ...
            return None, (
                jsonify({"error": "Лимит времени должен быть целым числом"}),
                400,
            )
        if not (1 <= time_limit_sec <= MAX_CATEGORY_TIME_LIMIT_SEC):
            return None, (
                jsonify(
                    {
                        "error": f"Лимит времени должен быть от 1 до {MAX_CATEGORY_TIME_LIMIT_SEC} секунд"
                    }
                ),
                400,
            )
